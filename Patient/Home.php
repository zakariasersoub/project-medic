...
// جلب معرف المريض المرتبط بهذا المستخدم
$patient_id = 0;
if ($user_id > 0) {
    $result = $conn->query("SELECT id FROM patients WHERE user_id = $user_id LIMIT 1");
    if ($row = $result->fetch_assoc()) {
        $patient_id = intval($row['id']);
    }
}
...
// عدد المواعيد القادمة لهذا المريض فقط
$appointments = 0;
if ($patient_id > 0) {
    $appointments = $conn->query("SELECT COUNT(*) as total FROM appointments WHERE scheduled_time >= NOW() AND patient_id = $patient_id")->fetch_assoc()['total'];
}

// جلب آخر 3 مواعيد قادمة لهذا المريض فقط مع صور المرضى والأطباء
$appointments_result = $conn->query("
    ...
    WHERE a.scheduled_time >= NOW() AND a.patient_id = $patient_id
